...
if __name__ == '__main__':
	...
	while keepGoing == 1:
		...
		if(version != 'x'):
			...
			print "LoadParseTimes: listOfAgents:%f, simStats:%f"%(stage_loadAgents - stage_start, stage_loadSimulationStats - stage_loadAgents)
			stage_start = time.clock()	
			worldMaps.drawAgents(window,mapProperties.mapProperties(0,0, 0,0, 200,200, 3),listOfAgents) #600 x 600, spacing = 3
			stage_zoomView = time.clock()	
			worldMaps.drawAgents(window,mapProperties.mapProperties(650,50, 0,0, 400,400, 0),listOfAgents) #600 x 600, spacing = 3
